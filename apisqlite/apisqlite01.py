...
def movielookup(mykey, searchstring,types="",years=""):
    """Interactions with OMDB API
       mykey = omdb api key
       searchstring = string to search for"""
    try:
        # begin constructing API
        api = f"{OMDBURL}apikey={mykey}&s={searchstring}&type={types}&y={years}"
    ...
    while True:
        ...
        if answer in ["1", "2","3"]:
            ...
            if answer == "1":
                ...
                #continue
                resp = movielookup(mykey,searchstring,"movie")
            elif answer == "3":
                years =input("Enter the four digit year the movie was released: ").strip()
                resp= movielookup(mykey=mykey,searchstring=searchstring,years=years)
